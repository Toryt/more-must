...
const assertionErrorTest = require('must/test/must/_assertion_error_test')
...
module.exports = function (callToTest) {
  it('must fail given null', function () {
    assert.fail(function () {
      callToTest(Must(null))
    })
  })

  it('must fail given undefined', function () {
    assert.fail(function () {
      callToTest(Must(undefined))
    })
  })

  it('must fail given boolean primitive', function () {
    assert.fail(function () {
      callToTest(Must(true))
    })
    assert.fail(function () {
      callToTest(Must(false))
    })
  })
  ...
  describe('.not', function () {
    it('must invert the assertion', function () {
      assert.fail(function () {
        callToTest(Must(catchAndThen).not)
      })
    })
  })
}
